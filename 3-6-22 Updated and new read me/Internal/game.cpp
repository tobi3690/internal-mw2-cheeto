...
namespace g_game
{
	...
	Vector3 get_camera_location()
	{
		...
		return *(Vector3*)(camera + offsets::camera_pos);
	...
	int get_local_index() {
		auto local_index = *reinterpret_cast<uintptr_t*>(sdk::get_client_info() + offsets::local_index);
	...
	{
		auto ret = sdk::get_client_info_base() + (index * offsets::player::size);
		if (is_bad_ptr(ret)) return 0;
		return ret;
	}
	...
	bool _is_dead(uintptr_t base) {
		auto dead1 = *reinterpret_cast<bool*>((uintptr_t)base + offsets::player::dead_1);
		auto dead2 = *reinterpret_cast<bool*>((uintptr_t)base + offsets::player::dead_2);
		return dead1 || dead2;
	}

	bool _is_valid(uintptr_t base) {

		return *reinterpret_cast<bool*>((uintptr_t)base + offsets::player::valid);
	}

	int _stance(uintptr_t base) {

		return *reinterpret_cast<int*>((uintptr_t)base + offsets::player::stance);
	}
	DWORD color = 0x0;
	int called = 0;
